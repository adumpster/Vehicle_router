...
static void parse_employees_from_input(
    ...
    if (!je.is_object()) throw std::runtime_error("input.employees not an object");
...
static void parse_vehicles_from_input(
    ...
    if (!jv.is_array()) throw std::runtime_error("input.vehicles not an array");
...
static void reconstruct_routes(
    ...
    for (const auto& jv : jvehs.arr) {
        ...
        Vehicle* veh = nullptr;
        for (auto& v : vehs) if (v.id == vid) { veh = &v; break; }
        if (!veh) continue;

        const auto& jtrips = jv["trips"];
        if (!jtrips.is_array()) continue;
        ...
        veh->total_cost = 0.0;
        for (const auto& r : veh->routes) veh->total_cost += r.total_cost;
    }
}
...
static bool resimulate(
    ...
    if (stops.size() < 2) { fail_reason = "route too short"; return false; }
    ...
    for (int i = 1; i + 1 < (int)stops.size(); i++) {
        ...
        if (it == emp_map.end()) continue;
        const Employee& e = *it->second;
        if (office_arr > e.due_time) {
            fail_reason = "due_time violated for " + e.id
                          + " (office=" + format_time(office_arr)
                          + " due=" + format_time(e.due_time) + ")";
            return false;
        }
    }
...
static int eff_cap(const Vehicle& veh, const Route& route,
    ...
    if (route.max_capacity > 0) cap = std::min(cap, route.max_capacity);
    cap = std::min(cap, emp_share_limit(emp.share_pref));
    return cap;
}
...
static bool is_compat(const Vehicle& veh, const Route& route,
                      const Employee& emp) {
    if (emp.veh_pref == PREMIUM && veh.category != PREMIUM) return false;
    if (emp.veh_pref == NORMAL  && veh.category == PREMIUM) return false;
    if (route.current_capacity + 1 > eff_cap(veh, route, emp)) return false;
    return true;
}
...
static bool insert_existing(
    ...
    for (int vi = 0; vi < (int)vehicles.size(); vi++) {
        ...
        for (int ri = 0; ri < (int)veh.routes.size(); ri++) {
            ...
            if (route.stops.back().emp_id != "END") continue;
    ...
    route.max_capacity = std::min(route.max_capacity,
                                  eff_cap(veh, route, emp));
    ...
    for (auto& e : employees)
        if (e.id == emp.id) { e.is_routed = true; break; }
...
static bool insert_new_trip(
    ...
    for (int vi = 0; vi < (int)vehicles.size(); vi++) {
        ...
        if (emp.veh_pref == PREMIUM && veh.category != PREMIUM) continue;
        if (emp.veh_pref == NORMAL  && veh.category == PREMIUM) continue;
        if (emp_share_limit(emp.share_pref) < 1) continue;
    ...
    nr.max_capacity     = std::min((int)veh.capacity,
                                   emp_share_limit(emp.share_pref));
    ...
    for (size_t vi = 0; vi < vehs.size(); vi++) {
        ...
        for (size_t ti = 0; ti < v.routes.size(); ti++) {
            ...
            for (size_t pi = 0; pi < pax.size(); pi++) {
                ...
                int pu_t = 0;
                for (const auto& s : r.stops)
                    if (s.is_pickup && s.emp_id == eid) { pu_t = s.departure_time; break; }
                if (pi) f << ",\n";
...
DynResult run_dynamic_insertion(
    ...
    if (debug) std::cout << "[DynamicHandler] Reading solved output: "
                         << solved_output_path << "\n";
    ...
    emit_json = [&](std::ostream& os, const mini_json::Value& v) {
        ...
        switch (v.type) {
            ...
            case T::Bool:   os << (v.b ? "true" : "false"); break;
            case T::Number: os << std::setprecision(15) << v.num; break;
            case T::String: os << "\"" << json_esc(v.str) << "\""; break;
            case T::Array: {
                os << "[";
    ...
    if (debug) {
        int routed = 0;
        for (const auto& e : employees) if (e.is_routed) routed++;
    ...
    if (debug) std::cout << "[DynamicHandler] Reading new employees: "
                         << new_emps_path << "\n";
    ...
    for (const auto& kv : jne.obj) {
        ...
        bool exists = false;
        for (const auto& e : employees) if (e.id == id) { exists = true; break; }
        if (exists) {
            if (debug)
    ...
    if (new_emps.empty()) {
        if (debug) std::cout << "[DynamicHandler] No new employees to insert.\n";
        ...
        if (debug)
            std::cout << "\n[" << emp.id << "] ready=" << format_time(emp.ready_time)
                      ...
                      << (emp.veh_pref == PREMIUM ? "premium"
                          : emp.veh_pref == NORMAL  ? "normal" : "any")
                      << " share="
                      << (emp.share_pref == SINGLE ? "single"
                          : emp.share_pref == DOUBLE ? "double"
                          : emp.share_pref == TRIPLE ? "triple" : "any")
                      << "\n";

        if (debug) std::cout << "  [Pass 1] checking existing routes...\n";
        if (insert_existing(emp, employees, vehicles, outcome, debug)) {
            result.outcomes.push_back(outcome);
            continue;
        }

        if (debug) std::cout << "  [Pass 2] opening new trip...\n";
    ...
    if (debug) {
        int ok = 0;
        for (const auto& o : result.outcomes) if (o.inserted) ok++;
        std::cout << "\n[DynamicHandler] " << ok << "/"
