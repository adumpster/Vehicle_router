...
// ---------------------------------------------------------------
// SHARING POLICY (consistent with alns.cpp and infeasible_handler.cpp):
//   SINGLE    → ride alone; effective cap = 1.
//   DOUBLE    → share with at most 1 other; effective cap = 2.
//   TRIPLE    → share with at most 2 others; effective cap = 3.
//   ANY_SHARE → no numeric restriction beyond vehicle capacity.
//
// VEHICLE PREFERENCE is a HARD rule:
//   veh_pref == PREMIUM → only premium vehicles accepted.
//   veh_pref == NORMAL  → premium vehicles are blocked
//                          (employee explicitly wants non-premium).
// ---------------------------------------------------------------

// Returns the maximum number of passengers allowed by this sharing preference.
static int sharing_cap_val(SharingPref p)
{
    switch (p)
    {
...
bool check_compatibility(const Vehicle &v, const Employee &e, const Route &route)
{
    ...
    if (e.veh_pref == PREMIUM && v.category != PREMIUM)
        return false;
    if (e.veh_pref == NORMAL && v.category == PREMIUM)
        return false;

    // Sharing cap enforced for all preference types.
    int emp_limit = min((int)v.capacity, sharing_cap_val(e.share_pref));

    // route.max_capacity encodes the most restrictive sharing cap of current passengers.
    int effective_capacity = min(route.max_capacity, emp_limit);
    if (route.current_capacity + 1 > effective_capacity)
        return false;
...
void solve_solomon_insertion(vector<Employee> &emps, vector<Vehicle> &vehs, bool debug)
{
    ...
    for (size_t order_idx = 0; order_idx < sorted_indices.size(); order_idx++)
    {
        ...
        if (best_vehicle_idx != -1)
        {
            ...
            // Update route capacity limit: narrow by this employee's sharing preference.
            int emp_limit = min((int)veh.capacity, sharing_cap_val(emps[emp_idx].share_pref));
            route.max_capacity = min(route.max_capacity, emp_limit);
        ...
        else
        {
            ...
            for (auto &v : vehs)
            {
                ...
                // Sharing cap enforced for all preference types.
                int emp_limit = min((int)v.capacity, sharing_cap_val(emps[emp_idx].share_pref));

                if (emp_limit == 0)
                    continue;
                if (emps[emp_idx].veh_pref == PREMIUM && v.category != PREMIUM)
                    continue;
                if (emps[emp_idx].veh_pref == NORMAL && v.category == PREMIUM)
                    continue;
