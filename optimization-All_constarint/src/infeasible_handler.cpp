...
{
    if (emp.veh_pref == PREMIUM && v.category != PREMIUM) return false;
    if (emp.veh_pref == NORMAL  && v.category == PREMIUM) return false;
    return true;
}
...
{
    int cap = std::min({(int)v.capacity, route.max_capacity, sharing_cap(emp)});
...
make_lookup(const std::vector<Employee> &employees)
{
    ...
    for (const auto &e : employees) m[e.id] = &e;
    return m;
}
    ...
    const std::map<std::string, const Employee *> &lookup)
{
    ...
    {
        int arrive = cand[i-1].departure_time
                   + travel_minutes(get_dist(cand[i-1], cand[i]), v.speed_kmh);
        ...
        else
        {
            ...
            if (it == lookup.end()) return std::numeric_limits<int>::max();
    ...
    int max_slack)
{
    ...
    auto feasible = [&](int s) -> bool
    {
        for (int pos = 1; pos <= n - 1; ++pos)
        {
            ...
            if (office_arr == std::numeric_limits<int>::max()) continue;
            if (office_arr > emp.due_time + s) continue;
            ...
            for (int i = 1; i + 1 < (int)route.stops.size(); ++i)
            {
                ...
                if (office_arr > it->second->due_time) { ok = false; break; }
            }
            if (ok) return true;
        }
        return false;
    ...
    if (!feasible(max_slack)) return -1;
    ...
    int slack)
{
    ...
    for (int pos = 1; pos <= n - 1; ++pos)
    {
        ...
        for (int i = 1; i < (int)cand.size(); ++i)
        {
            ...
            {
                auto it = lookup.find(cand[i].emp_id);
                if (it == lookup.end()) { ok = false; break; }
                int begin = std::max(arrive, it->second->ready_time);
                cand[i].begin_service  = begin;
                cand[i].departure_time = begin + SERVICE_MIN;
            }
        ...
        if (office_arr > emp.due_time + slack) continue;

        for (int i = 1; i + 1 < (int)cand.size() && ok; ++i)
        {
            if (cand[i].emp_id == emp.id) continue;
            auto it = lookup.find(cand[i].emp_id);
            if (it == lookup.end()) continue;
            if (office_arr > it->second->due_time) ok = false;
        }
        if (!ok) continue;
        ...
        double cost = W_COST * (dist * v.cost_per_km) + W_TIME * (t);
        if (cost < best_cost) { best_cost = cost; best_stops = cand; }
    }
        ...
        route.total_distance += get_dist(route.stops[i-1], route.stops[i]);
    {
        ...
        route.total_cost = W_COST * (route.total_distance * v.cost_per_km)
                         + W_TIME * (t);
    }
    return true;
}
...
{
    Route r;
    r.max_capacity     = std::min((int)v.capacity, sharing_cap(emp));
    ...
    InfeasibleResult &res)
{
    ...
    {
        struct RouteCand { int vi, ri, min_slack; double cost_delta; };
        ...
        for (int vi = 0; vi < (int)vehicles.size(); ++vi)
        {
            ...
            {
                auto &r = v.routes[ri];
                if (!capacity_ok(r, v, emp)) continue;
        ...
        if (!route_cands.empty())
        {
            std::sort(route_cands.begin(), route_cands.end(),
                      ...
                      {
                          if (a.min_slack != b.min_slack) return a.min_slack < b.min_slack;
    ...
    {
        struct TripCand { int vi, min_slack, arrive_pickup; double trip_cost; };
    ...
    const std::map<int, int>    &priority_budget)
{
    ...
    for (auto &emp : employees)
    {
        ...
        if (it != priority_budget.end()) budget = it->second;
    ...
    for (auto &cand : queue)
    {
        ...
        res.budget_overridden = (cand.slack_needed != std::numeric_limits<int>::max())
                                 && (cand.slack_needed > cand.budget);
    ...
    const std::vector<std::pair<std::string, int>> &metadata_kv)
{
    ...
    for (const auto &kv : metadata_kv)
    {
        ...
        if (key.rfind("priority_", 0) != 0) continue;
        if (key.find("_max_delay_min") == std::string::npos) continue;
