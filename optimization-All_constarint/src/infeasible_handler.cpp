...
{
    if (emp.veh_pref == PREMIUM && v.category != PREMIUM)
        return false;
    if (emp.veh_pref == NORMAL && v.category == PREMIUM)
        return false;
    return true;
}
    ...
    int slack)
{
        ...
        route.total_distance += get_dist(route.stops[i - 1], route.stops[i]);
    {
        ...
        route.total_cost = W_COST * (route.total_distance * v.cost_per_km) + W_TIME * (t);
    }
    return true;
}
